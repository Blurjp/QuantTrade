...
def generate_historical_signals(
    ...
    if signal_type == "chokepoint":
    ...
    elif signal_type in ["agricultural", "oil_storage", "auto_inventory"]:
        # NDVI-based signals
        if 'ndvi_mean' in df.columns:
            ...
            if signal_type == "auto_inventory":
                df['signal_direction'] = np.where(df['signal_raw'] < -0.1, 'long',
                                                  np.where(df['signal_raw'] > 0.1, 'short', 'neutral'))
            else:
